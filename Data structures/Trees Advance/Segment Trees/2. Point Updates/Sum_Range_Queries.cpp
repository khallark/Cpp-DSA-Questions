...
int updateQuery(int i, int point, int val, int l, int r) {
    ...
    int mid = l + (r - l) / 2;
    if(point <= mid) {
        segTree[i] = updateQuery(2 * i + 1, point, val, l, mid) + segTree[2 * i + 2];
    } else {
        segTree[i] = segTree[2 * i + 1] + updateQuery(2 * i + 2, point, val, mid + 1, r);
    }
    return segTree[i];
}
...
int main() {
    ...
    REP(i, 0, q) {
        ...
        if(mode) {
            ...
            cin >> l >> r;
            ans.push_back(sumQuery(0, l, r, 0, n - 1)); // O(log n)
        }
    }
